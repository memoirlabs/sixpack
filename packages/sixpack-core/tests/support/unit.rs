...
#[test]
fn filesystem_backed_names_are_validated() {
    assert!(WorkspaceName::new("notes-db").is_ok());
    assert!(WorkspaceName::new("../escape").is_err());
    assert!(WorkspaceName::new("/absolute").is_err());
