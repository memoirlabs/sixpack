...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("note-taking playground http://{}/", listener.local_addr()?);
    println!(
        "database {}",
        app.lock()
...
impl Config {
    fn from_args() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--out" => {
                    ...
                    let root = PathBuf::from(args.next().ok_or("--out requires a value")?);
                    database = Some(root.join("notes-db"));
                }
                "--reset" => reset = true,
                "-h" | "--help" => {
                    println!(
                        "note-taking-playground [--database <path>] [--host <host>] [--port <port>] [--reset]"
                    );
                    println!();
        ...
        Ok(Self {
            host,
            port,
            database: database.unwrap_or_else(temporary_database_path),
            reset,
        })
    }
}
...
impl App {
    fn open(database: PathBuf) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        db.init()?;
        let mut app = Self { db, revision: 0 };
        app.ensure_notebook()?;
        if app.list_notes()?.is_empty() {
            app.seed()?;
        }
        app.write_projection()?;
        Ok(app)
    }
    ...
    fn seed(&mut self) -> Result<Timing, Box<dyn std::error::Error>> {
        ...
        Ok(Timing::write_and_projection(write_elapsed, self.bump()))
    }
    ...
    fn create_note(
        ...
        Ok(MutationResult {
            note,
            timings: Timing::write_and_projection(write_elapsed, self.bump()),
        })
    }
    ...
    fn write_projection(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.db.write_projection()?;
        Ok(())
    }

    fn bump(&mut self) -> ProjectionTiming {
        self.revision = self.revision.saturating_add(1);
        let started = Instant::now();
        match self.write_projection() {
            Ok(()) => ProjectionTiming {
...
impl Timing {
    fn write_and_projection(write: Duration, projection: ProjectionTiming) -> Self {
        Self {
            read_ms: None,
            write_ms: Some(write.as_secs_f64() * 1_000.0),
...
mod tests {
    ...
    #[test]
    fn projection_failure_does_not_misreport_a_committed_write() {
        let root = temp_root();
        let mut app = App::open(root.clone()).unwrap();
        let blocker = app.database_path().join("projection.html");
        fs::remove_file(&blocker).unwrap();
        fs::create_dir(&blocker).unwrap();

        let created = app
            .create_note(NoteInput {
                title: Some("Committed note".to_owned()),
                body: Some("Projection failure is derived-state only".to_owned()),
            })
            .unwrap();

        assert!(created.timings.projection_ms.is_none());
        assert!(created.timings.projection_error.is_some());
        assert!(
            app.list_notes()
                .unwrap()
                .into_iter()
                .any(|note| note.id == created.note.id)
        );
