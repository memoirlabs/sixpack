...
/// Emits a typed TypeScript schema module for `@sixpack/db`.
///
/// Integer fields use TypeScript `bigint` values for an exact Rust `i64`
/// mapping. Generated applications can use `int64(...)` to convert ordinary
/// safe numbers or decimal strings at their input boundaries.
pub fn emit_typescript(ir: &SchemaIr) -> String {
    let mut out = String::new();
    out.push_str("// Generated by sixpack. Do not edit by hand.\n");
    out.push_str("import { createTable, type SixpackSchema } from \"@sixpack/db\";\n\n");

    for table in &ir.tables {
        let row_name = format!("{}Row", pascal_case(&table.name));
        out.push_str(&format!("export interface {row_name} {{\n"));
        for field in &table.fields {
            out.push_str(&format!(
                "  readonly {}: {};\n",
                field.name,
                typescript_type_name(field.ty)
            ));
        }
        out.push_str("}\n\n");

        out.push_str(&format!(
            "export const {} = createTable<{row_name}>()({{\n",
            table.name
        ));
        ...
        for field in &table.fields {
            out.push_str(&format!(
                "    {}: \"{}\",\n",
                field.name,
                <&'static str>::from(field.ty)
            ));
        }
        out.push_str("  },\n");
        out.push_str("  lookups: {\n");
        out.push_str("    id: { kind: \"id\", unique: true },\n");
        for lookup in &table.lookups {
            let field = table
                .fields
                .iter()
                .find(|field| field.name == lookup.field_name)
                .expect("validated lookups reference declared fields");
            out.push_str(&format!(
                "    {}: {{ kind: \"{}\", unique: {} }},\n",
                lookup.field_name,
                <&'static str>::from(field.ty),
    ...
    out.push_str("export const schema = {\n");
    ...
    out.push_str("  },\n");
    out.push_str("} satisfies SixpackSchema;\n");
    out
}

fn typescript_type_name(primitive: PrimitiveType) -> &'static str {
    match primitive {
        PrimitiveType::Id | PrimitiveType::Text => "string",
...
mod tests {
    ...
    #[test]
    fn emits_typed_typescript_tables_and_exact_integers() {
        let source = "schema! { users { id id \n email text \n login_count int \n trust float \n disabled bool \n lookup email unique \n } }";
        let ir = compile_schema(source).unwrap();
        let code = emit_typescript(&ir);

        assert!(code.contains("export interface UsersRow"));
        assert!(code.contains("login_count: bigint"));
        assert!(code.contains("trust: number"));
        assert!(code.contains("disabled: boolean"));
        assert!(code.contains("email: { kind: \"text\", unique: true }"));
        assert!(code.contains("createTable<UsersRow>()"));
        assert!(code.contains("satisfies SixpackSchema"));
    }

    #[test]
    fn typescript_chat_schema_matches_reviewed_output() {
        let source = include_str!("../../sixpack/examples/chat_schema.sixpack");
        let expected = include_str!("../../sixpack-typescript/test/chat-schema.ts");
        let ir = compile_schema(source).unwrap();

        assert_eq!(emit_typescript(&ir), expected);
    }
